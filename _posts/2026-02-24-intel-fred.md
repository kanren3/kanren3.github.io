...
- 如果中断异常向量对应的 **IDT Entry** 的 **IST** 非零，则会无条件将 **RSP** 切换至 **TSS** 中 **IST** 字段对应的值。
- 否则就检查是否产生权限跃迁，如果产生了权限跃迁，则会将 **RSP** 切换至 **TSS** 中 **RSP** 字段对应的值。
- 否则不会进行栈切换。
...
随后通过 `newCSL = MAX(oldCSL, eventSL)` 选出 **newCSL**，然后进行根据以下因素决定是否进行栈切换：
