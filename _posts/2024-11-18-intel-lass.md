...
- 置位状态下，代表当前页面属于 **User Mode**。
- 复位状态下，代表当前页面属于 **Supervisor Mode**。
...
**SMAP** 和 **SMEP** 虽然保证了内存的访问权限，但是仍然无法逃避掉 **Page Walk**，而在这个过程中，攻击者可以通过大量的内存访问操作来统计时间信息，从而间接分析页表的分页结构等信息，于是便诞生了 **Intel LASS**。
...
当 **LASS** 启用以后，是否可以使用此虚拟地址寻址，是由虚拟地址的 **最高有效位** 来决定的，而曾经 **Page Walk** 中越权所产生的 **#PF** 异常会转变为 **#GP** 和 **#SS**，操作系统有义务添加对于这种异常的处理，并且在寻址前 **CR4.SMAP** 与 **CR4.SMEP** 的作用也发生了一些微小的变化：

- **CR4.SMEP** 无论是否置位，一律会视为置位状态，**Supervisor Mode** 永远不可以对 **最高有效位** 为 0 的虚拟地址进行取指。
- **CR4.SMAP** 则可以控制 **Supervisor Mode** 是否可以对 **最高有效位** 为 0 的虚拟地址进行寻址访问。
  - 复位状态下，完全不可以进行寻址访问。
  - 置位状态下，和曾经一样，通过 **EFLAGS.AC** 标志来决定是否可以进行显式的寻址访问。

而 **User Mode** 则永远无法对 **最高有效位** 为 1 的虚拟地址进行访问，理论上这本就不可能。
